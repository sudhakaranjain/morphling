...
class mBertBaseline(pl.LightningModule):
    def __init__(self, model_hparams):
        super(mBertBaseline, self).__init__()

        self.model_hparams = model_hparams
        self.mbert = AutoModel.from_pretrained(self.model_hparams['base_model'])
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor, token_type_ids: torch.tensor, task) -> torch.tensor:

        features = self.mbert(input_ids, attention_mask, token_type_ids)
    ...
    def training_step(self, batch: torch.tensor, batch_idx: int) -> torch.tensor:
        ...
        task = batch['task']
        # Forward pass
        outputs = self(input_ids, attention_mask, token_type_ids, task)
    ...
    def configure_optimizers(self) -> torch.optim.Adam:
        ...
        return torch.optim.Adam(self.parameters(), lr=self.model_hparams['learning_rate'])
    ...
    def predict_udpos(self, filename: str) -> List[List[str]]:
        with torch.no_grad():
            ...
            for batch in batches:
                ...
                logits = self(encodings['input_ids'], encodings['attention_mask'], encodings['token_type_ids'], "udpos")
    ...
    def predict_panx(self, filename: str) -> List[List[str]]:
        with torch.no_grad():
            ...
            for batch in batches:
                encodings_orig = tokenizer(batch, truncation=True, padding=True, max_length=512, return_tensors='pt', is_split_into_words=True)
                encodings = {key: torch.tensor(val).to(device) for key,val in encodings_orig.items()}
                logits = self(encodings['input_ids'], encodings['attention_mask'], encodings['token_type_ids'], "panx")
                logits = logits.to('cpu')
                for k,sent in enumerate(logits):
                    word_ids = encodings_orig.word_ids(batch_index=k)
                    label = []
    ...
    def predict_xnli(self, filename: str) -> List[str]:
        with torch.no_grad():
            ...
            for batch_1, batch_2 in zip(batches_1, batches_2):
                ...
                logits = self(encodings['input_ids'], encodings['attention_mask'], encodings['token_type_ids'], "xnli")
                logits = logits.to('cpu')
                for logit in logits:
    ...
    def predict_pawsx(self, filename: str) -> List[str]:
        with torch.no_grad():
            ...
            for batch_1, batch_2 in zip(batches_1, batches_2):
                encodings = tokenizer(batch_1, batch_2, truncation=True, padding=True, max_length=512, return_tensors='pt')
                encodings = {key: torch.tensor(val).to(device) for key,val in encodings.items()}
                logits = self(encodings['input_ids'], encodings['attention_mask'], encodings['token_type_ids'], "pawsx")
                logits = logits.to('cpu')
                for logit in logits:
...
def load_and_preprocess_data():
    ...
    for task in task_list:
        ...
        if task == 'udpos':
            ...
            sentences_train, sentences_val, labels_train, labels_val = train_test_split(sentences, labels, test_size=0.20, random_state=42)
            ...
            df_xnli_train, df_xnli_val = train_test_split(df, test_size=0.20, random_state=42)
            ...
            df = pd.concat(df_chunks, ignore_index=True)  # contains data of current task
            df.dropna(inplace=True)
            dict_train[task], dict_val[task] = train_test_split(df, test_size=0.20, random_state=42)
...
if __name__ == "__main__":
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename='mBert-baseline-finetuned-{epoch:02d}-{val_acc:.2f}',
        mode="min",
    )
